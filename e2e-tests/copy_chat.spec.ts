...
test("copy message content - basic functionality", async ({ po }) => {
  ...
  expect(clipboardContent).not.toContain("<orianbuilder-");
});

test("copy message content - orianbuilder-write conversion", async ({ po }) => {
  await po.setUp({ autoApprove: true });
  await po.importApp("minimal");
  ...
  // Should convert orianbuilder-write to markdown format (flexible path matching)
  expect(clipboardContent).toContain("### File:");
  expect(clipboardContent).toContain("```");
  expect(clipboardContent).not.toContain("<orianbuilder-write");
});
