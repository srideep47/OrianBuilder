...
const testWithRemoteCatalog = testWithConfig({
  preLaunchHook: async ({ fakeLlmPort }) => {
    process.env.ORIANBUILDER_LANGUAGE_MODEL_CATALOG_URL = `http://localhost:${fakeLlmPort}/api/language-model-catalog`;
  },
...
const testWithFallbackCatalog = testWithConfig({
  preLaunchHook: async ({ fakeLlmPort }) => {
    process.env.ORIANBUILDER_LANGUAGE_MODEL_CATALOG_URL = `http://localhost:${fakeLlmPort}/missing-language-model-catalog`;
  },
  postLaunchHook: async () => {
    delete process.env.ORIANBUILDER_LANGUAGE_MODEL_CATALOG_URL;
  },
