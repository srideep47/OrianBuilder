...
/**
 * A webpack loader that adds `data-orianbuilder-*` attributes to JSX elements.
 */
export default function orianbuilderTaggerLoader(this: any, code: string) {
  ...
  const transform = async () => {
    try {
          ...
          try {
            ...
            const alreadyTagged = node.attributes?.some(
              (attr: any) =>
                ...
                attr.name?.name === "data-orianbuilder-id",
            );
            if (alreadyTagged) return;

            // Build the orianbuilder ID
            const loc = node.loc?.start;
            if (!loc) return;
            const orianbuilderId = `${fileRelative}:${loc.line}:${loc.column}`;

            // Inject the attributes
            if (node.name.end != null) {
              ms.appendLeft(
                node.name.end,
                ` data-orianbuilder-id="${orianbuilderId}" data-orianbuilder-name="${tagName}"`,
              );
              transformCount++;
            }
          } catch (error) {
            console.warn(
              `[orianbuilder-tagger] Warning: Failed to process JSX node in ${this.resourcePath}:`,
              error,
            );
          }
        },
      ...
      console.warn(
        `[orianbuilder-tagger] Warning: Failed to transform ${this.resourcePath}:`,
        error,
      );
      return null;
    }
  ...
  transform()
    ...
    .catch((err) => {
      console.error(
        `[orianbuilder-tagger] ERROR in ${this.resourcePath}:`,
        err,
      );
