 ...
 */
export default function orianbuilderTagger(): Plugin {
  return {
    name: "vite-plugin-orianbuilder-tagger",
    apply: "serve",
    enforce: "pre",
    ...
    async transform(code: string, id: string) {
      try {
        ...
        walk(ast as any, {
          enter(node: any) {
            try {
              ...
              // ── 2. Check whether the tag already has data-orianbuilder-id ───────────────
              ...
              console.warn(
                `[orianbuilder-tagger] Warning: Failed to process JSX node in ${id}:`,
                error,
              );
            }
          },
        ...
        console.warn(
          `[orianbuilder-tagger] Warning: Failed to transform ${id}:`,
          error,
        );
        return null;
      }
