# Add Convex Backend Support to OrianBuilder

> Generated by swarm planning session on 2026-02-12

## Summary

Add Convex as a third backend option in OrianBuilder (alongside Supabase and Neon), enabling users to build apps with real-time reactive data, serverless functions, and file storage — all without SQL. This positions OrianBuilder as the long-term successor to Chef (the Convex-native builder in `chef/`), porting Chef's battle-tested prompts and patterns into OrianBuilder's backend-flexible architecture.

## Problem Statement

OrianBuilder users currently have two backend options: Supabase and Neon, both PostgreSQL-based. Users building apps that need real-time reactivity, serverless functions, or document-based data models are forced into SQL paradigms or must leave OrianBuilder entirely (using Chef or building manually). Adding Convex expands OrianBuilder's addressable use cases and aligns with the **Backend-Flexible** design principle.

Target users:

1. **Existing OrianBuilder users** wanting reactive, real-time apps without managing WebSocket infrastructure
...
- **Deploy key connection flow** — User pastes a Convex deploy key + deployment URL to connect an existing Convex project to a OrianBuilder app. "Open Convex Dashboard" link via `ipc.system.openExternalUrl` for easy key retrieval. Deploy key field uses `type="password"` with show/hide toggle. Inline validation errors (not toasts).
- **Convex-aware agent prompts** — High-fidelity port of Chef's `convexGuidelines.ts` (~960 lines) and `solutionConstraints.ts` for OrianBuilder's tool set and execution model. This is the highest-value item. Since OrianBuilder is the long-term Chef successor, invest in prompt quality (budget 3-4 days, not a quick copy).
...
- **Convex template** — Fork Chef's template (`chef/template/`) as starting point. Pre-wired with `convex/` directory, `convex.config.ts`, `ConvexProvider`, correct start scripts, `convex` + `concurrently` npm dependencies, Tailwind + shadcn/ui compatibility. Production-quality since OrianBuilder replaces Chef long-term.
...
- **Agent tools** — `get_convex_project_info`, `get_convex_schema`, `deploy_convex` for local-agent mode. Also `<orianbuilder-add-integration provider="convex">` support in agent responses for suggesting Convex when appropriate.
...
- **OAuth authentication** — Replace deploy key paste with OAuth flow via `oauth.orianbuilder.sh` (v2)
...
1. **As a OrianBuilder user**, I want to connect my existing Convex project to a OrianBuilder app so that I can build apps with real-time data and serverless functions.
2. **As a OrianBuilder user**, I want the AI agent to generate correct Convex code (schema, queries, mutations, actions) so that my app works without manual debugging.
3. **As a OrianBuilder user**, I want my Convex functions to auto-sync during development (via `npx convex dev`) so that I see live data in the preview immediately.
4. **As a OrianBuilder user**, I want to approve schema deployments before they push to production so that I understand what's changing.
5. **As a OrianBuilder user**, I want the AI to know my current Convex schema so that it generates code consistent with my existing tables and indexes.
...
4. OrianBuilder opens Convex Dashboard deploy key page in browser (via `ipc.system.openExternalUrl`) with inline instructions: "Copy your deployment URL and deploy key from the Convex Dashboard"
5. User pastes **deployment URL** and **deploy key** into two input fields
6. OrianBuilder validates the credentials (test API call)
...
Follows OrianBuilder's existing layered IPC pattern:
...
**Dual dev server**: Template's `package.json` uses `concurrently` to run both `npx convex dev` and `npx vite`. OrianBuilder's existing `startCommand` column supports custom start scripts — no changes to `app_handlers.ts` process management.
...
| Convex template repo | Fork of Chef's template, adapted for OrianBuilder: `convex/`, `ConvexProvider`, `concurrently` start scripts, Tailwind | Medium |
...
- [ ] Fork Chef's template (`chef/template/`), adapt for OrianBuilder: `convex/` directory, `convex.config.ts`, `ConvexProvider` setup, `concurrently --names "vite,convex"` in start script, `convex` + `concurrently` dependencies, Tailwind + shadcn/ui compatibility
...
- [ ] High-fidelity port of Chef's `convexGuidelines.ts` + `solutionConstraints.ts` to `src/prompts/convex_prompt.ts` — systematically replace WebContainer/Chef tool references with OrianBuilder equivalents (budget 3-4 days)
...
- [ ] Add `<orianbuilder-add-integration provider="convex">` support in agent responses
...
1. **Convex branding assets** — Do we need permission from Convex to use their logo in OrianBuilder? Check trademark guidelines.
2. **Template hosting** — Should the Convex template be a new GitHub repo (like existing templates) or a fork of Chef's template adapted for OrianBuilder?
...
5. **Chef feature parity roadmap** — Since OrianBuilder is the long-term Chef successor, define concrete feature parity milestones. Chef has capabilities OrianBuilder doesn't: WebContainer-based sandboxed execution, sharing/forking projects, social shares with thumbnails, Convex Components support (proseMirror, presence, resend). Plan a follow-up session after MVP ships to create a "Chef parity checklist."

## Decision Log

| Decision                                   | Reasoning                                                                                                                                                                                          | Alternatives Considered                                                                                                                                                                  |
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Deploy key paste for auth (v1)             | Simplest to implement, proven in Chef, no server-side OAuth work needed. OAuth deferred to v2.                                                                                                     | OAuth via oauth.orianbuilder.sh (too much server-side work for v1), CLI login (breaks OrianBuilder UX pattern), hybrid deep-link (middle ground — we DO deep-link to the Dashboard page) |
...
| Replace Chef long-term                     | OrianBuilder's Convex integration is the long-term successor to Chef. Invest in feature parity. Port prompts and patterns.                                                                         | Keep Chef and OrianBuilder complementary (duplicated effort), port Chef into OrianBuilder entirely (too large)                                                                           |
| High-fidelity prompt port (3-4 days)       | Chef's prompt references WebContainer and Chef-specific tools. Naive copy causes broken tool references. Since OrianBuilder replaces Chef, invest in prompt quality.                               | Verbatim copy (broken references), write from scratch (loses battle-tested content), quick adaptation (insufficient for Chef replacement goal)                                           |
...
| Template from Chef fork (step 1)           | Having a working template unblocks all subsequent testing. Chef's template is battle-tested for Convex.                                                                                            | Build from scratch (slower), use generic Convex starter (may not match OrianBuilder's needs)                                                                                             |
