...
Users building apps with OrianBuilder can inadvertently (or through prompt injection) execute destructive actions. Today, OrianBuilder's only defense is the consent banner ("Allow once / Always allow / Decline"), which users frequently bypass with auto-approve or "Always allow" settings. Once bypassed, there is **zero validation**:
...
The LLM is an untrusted actor. Prompt injection, hallucination, and model errors can generate destructive operations the user never intended. Auto-approve removes the last line of defense. Users trust OrianBuilder to help them build safely.
...
- As a user with auto-approve enabled, I want OrianBuilder to still warn me before executing destructive SQL so that I don't accidentally lose data.
...
- As a user adding dependencies, I want OrianBuilder to warn me if a package is known-malicious or has known vulnerabilities so that I don't introduce security issues into my app.
