# OrianBuilder Universal Platform Plan (Desktop + Web + Mobile)

> Generated by swarm planning workflow on 2026-02-15

## Summary

Evolve OrianBuilder from a desktop-only Electron runtime into a multi-platform product with one core experience available on desktop, web, and mobile. Preserve OrianBuilder's local-first strengths on desktop while introducing a secure cloud execution runtime that powers equivalent capabilities on web/mobile where local process and filesystem access are unavailable. Deliver this in phased releases with strict compatibility, observability, and rollback controls.

## Problem Statement

OrianBuilder's architecture currently assumes a privileged local host (Electron main process) that can mutate files, spawn processes, run git/npm/docker, handle native protocol links, and store secrets via OS APIs. That creates three constraints:

1. Users cannot use OrianBuilder from browsers, tablets, or phones.
...
The core user problem is cross-device continuity for AI app building without losing the trust, transparency, and production-grade workflows OrianBuilder already supports.

## Product Principles Alignment

This plan explicitly preserves OrianBuilder's principles:
...
4. `OrianBuilder Cloud Runtime Services`
   - Project FS service
   - Exec service
   - Git service
   - Streaming broker
...
| `orianbuilder://` protocol deep links                  | Different deep-link systems per platform                  | Unified auth/deeplink router abstraction                                        | Implement desktop protocol + web redirect + mobile deep links             | Auth loop bugs             | End-to-end deep-link integration tests                  |
