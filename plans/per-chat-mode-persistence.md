...
- [ ] Also fire the toast on fresh chat load when the stored mode is already known to be unavailable client-side (via existing signals: `isOrianBuilderProEnabled`, `useFreeAgentQuota`, `isOpenAIOrAnthropicSetup`), so users see the fallback explanation before they type.
