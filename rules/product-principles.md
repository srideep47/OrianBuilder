...
These principles guide feature design decisions in OrianBuilder. Reference them when planning new features (especially via `orianbuilder:swarm-to-plan`) to ensure consistency with the product's values.
...
Users build apps from prototype to production in OrianBuilder. Every feature should support that full lifecycle, not just the demo.
...
- **Code ownership**: Generated code uses standard frameworks (React, Next.js, Vite) and lives in the user's git repo. No proprietary runtime or OrianBuilder-specific dependencies in the output.

**Test**: Could a user hand off the generated project to a team that doesn't use OrianBuilder and have them maintain it? If not, the feature creates unacceptable lock-in.
...
OrianBuilder integrates with the tools developers already use. It doesn't try to own their toolchain.

- **Package managers**: OrianBuilder runs `npm install` and custom install commands but doesn't manage node_modules or lock files beyond what the user configures.
- **External services**: Supabase CLI, Docker — OrianBuilder detects and uses these tools but doesn't install, start, or stop them. Example: local Supabase support detects `supabase status` but the user runs `supabase start` themselves.
- **IDEs**: Generated code is standard — users can open it in VS Code, Cursor, or any editor alongside OrianBuilder.
...
Using OrianBuilder should feel good and like it's crafted with care. Small details compound into an experience people love.
...
- **Live preview**: Changes appear in real time as the AI writes code. The feedback loop between "ask" and "see" is immediate and visceral — it's the core of what makes OrianBuilder feel alive.
