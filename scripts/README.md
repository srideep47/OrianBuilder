...
1. `orianbuilder-{version}-1.x86_64.rpm` (Linux RPM)
2. `orianbuilder-{version}-full.nupkg` (Windows NuGet package)
3. `orianbuilder-{version}.Setup.exe` (Windows installer)
4. `orianbuilder-darwin-arm64-{version}.zip` (macOS Apple Silicon)
5. `orianbuilder-darwin-x64-{version}.zip` (macOS Intel)
6. `orianbuilder_{version}_amd64.deb` (Linux DEB)
