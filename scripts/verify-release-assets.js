...
async function verifyReleaseAssets() {
  try {
    ...
    const owner = "orianbuilder-sh";
    const repo = "orianbuilder";
    ...
    const response = await fetch(allReleasesUrl, {
      headers: {
        ...
        "User-Agent": "orianbuilder-release-verifier",
      },
    });
    ...
    const expectedAssets = [
      `orianbuilder-${normalizeVersionForPlatform(version, "rpm")}-1.x86_64.rpm`,
      `orianbuilder-${normalizeVersionForPlatform(version, "nupkg")}-full.nupkg`,
      `orianbuilder-${version}.Setup.exe`,
      `orianbuilder-darwin-arm64-${version}.zip`,
      `orianbuilder-darwin-x64-${version}.zip`,
      `orianbuilder_${normalizeVersionForPlatform(version, "deb")}_amd64.deb`,
      `orianbuilder_${version}_x86_64.AppImage`,
      "RELEASES",
    ];
