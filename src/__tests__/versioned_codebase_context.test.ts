...
describe("parseFilesFromMessage", () => {
  describe("orianbuilder-read tags", () => {
    it("should parse a single orianbuilder-read tag", () => {
      const input =
        '<orianbuilder-read path="src/components/Button.tsx"></orianbuilder-read>';
      const result = parseFilesFromMessage(input);
      expect(result).toEqual(["src/components/Button.tsx"]);
    });

    it("should parse multiple orianbuilder-read tags", () => {
      const input = `
        <orianbuilder-read path="src/components/Button.tsx"></orianbuilder-read>
        <orianbuilder-read path="src/utils/helpers.ts"></orianbuilder-read>
        <orianbuilder-read path="src/styles/main.css"></orianbuilder-read>
      `;
      const result = parseFilesFromMessage(input);
    ...
    it("should trim whitespace from file paths in orianbuilder-read tags", () => {
      const input =
        '<orianbuilder-read path="  src/components/Button.tsx  "></orianbuilder-read>';
      const result = parseFilesFromMessage(input);
      expect(result).toEqual(["src/components/Button.tsx"]);
    ...
    it("should skip empty path attributes", () => {
      const input = `
        <orianbuilder-read path="src/components/Button.tsx"></orianbuilder-read>
        <orianbuilder-read path=""></orianbuilder-read>
        <orianbuilder-read path="src/utils/helpers.ts"></orianbuilder-read>
      `;
      const result = parseFilesFromMessage(input);
      expect(result).toEqual([
    ...
    it("should handle file paths with special characters", () => {
      const input =
        '<orianbuilder-read path="src/components/@special/Button-v2.tsx"></orianbuilder-read>';
      const result = parseFilesFromMessage(input);
      expect(result).toEqual(["src/components/@special/Button-v2.tsx"]);
    });
  });

  describe("orianbuilder-code-search-result tags", () => {
    it("should parse a single file from orianbuilder-code-search-result", () => {
      const input = `<orianbuilder-code-search-result>
src/components/Button.tsx
    ...
    it("should parse multiple files from orianbuilder-code-search-result", () => {
      const input = `<orianbuilder-code-search-result>
src/components/Button.tsx
...
src/utils/helpers.ts
    ...
    it("should skip empty lines in orianbuilder-code-search-result", () => {
      const input = `<orianbuilder-code-search-result>
src/components/Button.tsx
...
src/utils/helpers.ts
    ...
    it("should handle multiple orianbuilder-code-search-result tags", () => {
      const input = `<orianbuilder-code-search-result>
src/components/Button.tsx
src/components/Input.tsx
</orianbuilder-code-search-result>
...
src/styles/main.css
  ...
  describe("mixed tags", () => {
    it("should parse both orianbuilder-read and orianbuilder-code-search-result tags", () => {
      const input = `
<orianbuilder-read path="src/config/app.ts"></orianbuilder-read>
...
<orianbuilder-read path="src/components/Header.tsx"></orianbuilder-read>
...
<orianbuilder-read path="src/config/site.ts"></orianbuilder-read>
...
`;
  ...
  describe("edge cases", () => {
    ...
    it("should handle malformed tags gracefully", () => {
      const input = `
<orianbuilder-read path="src/file1.ts"
<orianbuilder-code-search-result>
src/file2.ts
`;
      const result = parseFilesFromMessage(input);
    ...
    it("should handle nested angle brackets in file paths", () => {
      const input =
        '<orianbuilder-read path="src/components/Generic<T>.tsx"></orianbuilder-read>';
      const result = parseFilesFromMessage(input);
...
SRC/COMPONENTS/BUTTON.TSX
    ...
    it("should handle very long file paths", () => {
      ...
      const input = `<orianbuilder-read path="${longPath}"></orianbuilder-read>`;
      const result = parseFilesFromMessage(input);
...
describe("processChatMessagesWithVersionedFiles", () => {
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content:
            'I found this file: <orianbuilder-read path="src/old.ts"></orianbuilder-read>',
          providerOptions: {
            "orianbuilder-engine": {
      ...
      const chatMessages: ModelMessage[] = [
        {
          ...
          content: [
            {
              type: "text",
              text: 'Here is the file: <orianbuilder-read path="src/array.ts"></orianbuilder-read>',
            },
            {
              type: "text",
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "user",
          content:
            'Check this: <orianbuilder-read path="src/user-file.ts"></orianbuilder-read>',
        },
      ];
      const appPath = "/test/app";
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content:
            'File here: <orianbuilder-read path="src/no-commit.ts"></orianbuilder-read>',
          // No providerOptions
        },
        {
          role: "assistant",
          content:
            'Another file: <orianbuilder-read path="src/no-commit2.ts"></orianbuilder-read>',
          providerOptions: {
            // orianbuilder-engine not set
          },
        },
      ];
      const appPath = "/test/app";
  ...
  describe("parsing multiple file paths", () => {
    it("should process multiple files from orianbuilder-code-search-result", async () => {
      const { getFileAtCommit } = await import("@/ipc/utils/git_utils");
      const mockGetFileAtCommit = vi.mocked(getFileAtCommit);
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content: `<orianbuilder-code-search-result>
src/file1.ts
src/file2.ts
</orianbuilder-code-search-result>`,
          providerOptions: {
    ...
    it("should process mixed orianbuilder-read and orianbuilder-code-search-result tags", async () => {
      const { getFileAtCommit } = await import("@/ipc/utils/git_utils");
      const mockGetFileAtCommit = vi.mocked(getFileAtCommit);
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content: `
<orianbuilder-read path="src/file1.ts"></orianbuilder-read>
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content:
            'Missing file: <orianbuilder-read path="src/missing.ts"></orianbuilder-read>',
          providerOptions: {
            "orianbuilder-engine": {
              sourceCommitHash: "commit1",
            },
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content:
            'Error file: <orianbuilder-read path="src/error.ts"></orianbuilder-read>',
          providerOptions: {
            "orianbuilder-engine": {
              sourceCommitHash: "commit1",
            },
    ...
    it("should process multiple messages with different commits", async () => {
      ...
      const chatMessages: ModelMessage[] = [
        ...
        },
        {
          role: "assistant",
          content:
            'Here it is: <orianbuilder-read path="src/file1.ts"></orianbuilder-read>',
          providerOptions: {
            "orianbuilder-engine": {
              sourceCommitHash: "commit1",
            },
        ...
        },
        {
          role: "assistant",
          content:
            'Here it is again: <orianbuilder-read path="src/file1.ts"></orianbuilder-read>',
          providerOptions: {
            "orianbuilder-engine": {
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content:
            'Old version: <orianbuilder-read path="src/old.ts"></orianbuilder-read>',
          providerOptions: {
            "orianbuilder-engine": {
              sourceCommitHash: "abc123",
            },
