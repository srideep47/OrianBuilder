...
interface OrianBuilderAppMediaFolderProps {
  appName: string;
  appId: number;
  appPath: string;
  files: MediaFile[];
...
export function OrianBuilderAppMediaFolder({
  appName,
  appId,
  appPath,
  files,
  ...
  searchQuery,
}: OrianBuilderAppMediaFolderProps) {
  ...
  if (isOpen) {
    return (
      <>
        ...
        {previewFile && (
          <ImageLightbox
            imageUrl={buildOrianBuilderMediaUrl(appPath, previewFile.fileName)}
            alt={previewFile.fileName}
            filePath={`${appPath}/.orianbuilder/media/${previewFile.fileName}`}
