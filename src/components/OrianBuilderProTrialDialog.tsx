...
interface OrianBuilderProTrialDialogProps {
  isOpen: boolean;
  onClose: () => void;
}

export function OrianBuilderProTrialDialog({
  isOpen,
  onClose,
}: OrianBuilderProTrialDialogProps) {
  const handleStartTrial = () => {
    ipc.system.openExternalUrl(
      "https://academy.orianbuilder.sh/redirect-to-checkout?trialCode=1PRO30&utm_source=orianbuilder-app&utm_medium=app&utm_campaign=setup-dialog-v2",
    );
    onClose();
  };
  ...
  const handleLearnMore = () => {
    ipc.system.openExternalUrl(
      "https://www.orianbuilder.sh/pro?utm_source=orianbuilder-app&utm_medium=app&utm_campaign=setup-dialog-v2",
    );
  };
