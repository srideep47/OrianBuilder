...
/**
 * Accent color configuration for OrianBuilderCard components.
 * Maps to Tailwind color classes for border, background, and text.
 */
export type OrianBuilderAccentColor =
  | "blue"
  | "purple"
  | "violet"
  | "red"
...
interface OrianBuilderCardProps {
...
/**
 * Premium container for all OrianBuilder markdown action cards.
 ...
 */
export function OrianBuilderCard({
  children,
  state,
  accentColor = "blue",
  showAccent,
...
interface OrianBuilderCardHeaderProps {
...
/**
 * Header row for OrianBuilderCard. Contains a tinted icon circle and flexible content area.
 */
export function OrianBuilderCardHeader({
  icon,
  accentColor = "blue",
  children,
}: OrianBuilderCardHeaderProps) {
  return (
...
interface OrianBuilderBadgeProps {
  children: React.ReactNode;
  color?: OrianBuilderAccentColor;
}

/**
 * Small pill badge for labeling card types (e.g. "GREP", "Turbo Edit", "SQL").
 */
export function OrianBuilderBadge({
  children,
  color = "blue",
...
interface OrianBuilderExpandIconProps {
  isExpanded: boolean;
}
 ...
 */
export function OrianBuilderExpandIcon({
  isExpanded,
}: OrianBuilderExpandIconProps) {
  return (
    <ChevronRight
      size={16}
...
interface OrianBuilderStateIndicatorProps {
  state: CustomTagState;
  pendingLabel?: string;
  abortedLabel?: string;
 ...
 */
export function OrianBuilderStateIndicator({
  state,
  pendingLabel,
  abortedLabel,
  finishedLabel,
}: OrianBuilderStateIndicatorProps) {
  if (state === "pending") {
    return (
...
interface OrianBuilderCardContentProps {
  isExpanded: boolean;
  children: React.ReactNode;
  className?: string;
}
 ...
 */
export function OrianBuilderCardContent({
  isExpanded,
  children,
  className = "",
}: OrianBuilderCardContentProps) {
...
interface OrianBuilderFilePathProps {
  path: string;
}
 ...
 */
export function OrianBuilderFilePath({ path }: OrianBuilderFilePathProps) {
  if (!path) return null;
  return (
...
interface OrianBuilderDescriptionProps {
  children: React.ReactNode;
}
 ...
 */
export function OrianBuilderDescription({
  children,
}: OrianBuilderDescriptionProps) {
  return (
