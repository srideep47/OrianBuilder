...
import { OrianBuilderTokenSavings } from "./OrianBuilderTokenSavings";

interface OrianBuilderThinkProps {
  node?: any;
  children?: React.ReactNode;
}

export const OrianBuilderThink: React.FC<OrianBuilderThinkProps> = ({
  children,
  node,
}) => {
  ...
  const tokenSavingsMatch =
    typeof children === "string"
      ? children.match(
          /^orianbuilder-token-savings\?original-tokens=([0-9.]+)&smart-context-tokens=([0-9.]+)$/,
        )
      : null;
  ...
  // If it's token savings format, render OrianBuilderTokenSavings component
