...
import { isFactoryBuildChat } from "@/main/flow/factory_build_registry";
import {
...
export async function runAgentStream(
  ...
  try {
    ...
    const agentTools = buildAgentToolSet(ctx, {
      ...
      // Orion Factory builds already have their media generated by the pipeline
      // (LLM unloaded). Drop media tools so the coding pass doesn't load a media
      // model while the LLM is resident and OOM.
      disableMediaTools: isFactoryBuildChat(chat.id),
    });
    const mcpTools =
