...
export interface BuildAgentToolSetOptions {
  ...
  /**
   * If true, exclude the media-generation tools (image/audio/video/3d). Used for
   * Orion Factory builds, where the pipeline already generated all assets with
   * the LLM unloaded; letting the coding-pass agent generate media would load a
   * media model alongside the resident LLM and run out of RAM/VRAM.
   */
  disableMediaTools?: boolean;
}
...
/**
 * Media-generation tools. Excluded from Orion Factory builds (assets are
 * pre-generated by the orchestrated pipeline) to keep single-model residency —
 * see `disableMediaTools`.
 */
...
export function buildAgentToolSet(
  ...
  for (const tool of TOOL_DEFINITIONS) {
    ...
    // For Orion Factory builds, skip media-generation tools (assets are already
    // generated by the pipeline; generating here would break single-residency).
    if (options.disableMediaTools && MEDIA_GEN_TOOLS.has(tool.name)) {
      continue;
    }
