...
const copyFileSchema = z.object({
  from: z
    .string()
    .describe(
      "The source file path (can be a .orianbuilder/media path or a path relative to the app root)",
    ),
...
export const copyFileTool: ToolDefinition<z.infer<typeof copyFileSchema>> = {
  name: "copy_file",
  description:
    "Copy a file from one location to another. Can copy uploaded attachment files (from .orianbuilder/media) into the codebase, or copy files within the codebase.",
  ...
  buildXml: (args, _isComplete) => {
    ...
    return `<orianbuilder-copy from="${escapeXmlAttr(args.from)}" to="${escapeXmlAttr(args.to)}" description="${escapeXmlAttr(args.description ?? "")}"></orianbuilder-copy>`;
  },
