...
- You can perform multiple distinct search and replace operations within a single \`orianbuilder-search-replace\` call by providing multiple SEARCH/REPLACE blocks. This is the preferred way to make several targeted changes efficiently.
...
- ALWAYS make as many changes in a single 'orianbuilder-search-replace' call as possible using multiple SEARCH/REPLACE blocks.
- Do not use both \`orianbuilder-write\` and \`orianbuilder-search-replace\` on the same file within a single response.
...
Usage:
<orianbuilder-search-replace path="path/to/file.js" description="Brief description of the changes you are making">
<<<<<<< SEARCH
def calculate_total(items):
    sum = 0
=======
