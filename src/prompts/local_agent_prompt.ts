...
7. **Version control + delivery.** When the work is complete and verified:
   ...
   - **For Electron / desktop / Android installer requests:** call \`package_native_artifact\` first to produce the installer(s) and \`native-download-site/\` landing page. Then (if hosting was approved above) \`connect_github_repo\` → \`connect_vercel_project\` → \`github_pr action="autopilot"\` → \`deploy_preview provider="custom_command" deploy_directory="native-download-site"\` to publish the download page. Vercel free tier rejects assets >50MB — if the installer is larger, fall back to a release-style note in the final summary and recommend GitHub Releases manually.
