(() => {
  const OVERLAY_CLASS = "__orianbuilder_overlay__";
  let overlays = [];
  ...
  function updateOverlay(el, isSelected = false, isHighlighted = false) {
    ...
    const name = el.dataset.orianbuilderName || "<unknown>";
    const file = (el.dataset.orianbuilderId || "").split(":")[0];
    ...
    if (highlightedElement) {
      ...
      if (highlightedItem) {
        ...
        window.parent.postMessage(
          {
            type: "orianbuilder-component-coordinates-updated",
            coordinates: {
              top: rect.top,
              left: rect.left,
              width: rect.width,
  ...
  function removeOverlayById(componentId) {
    ...
    overlays.forEach((item, index) => {
      if (item.el.dataset.orianbuilderId === componentId) {
    ...
    if (
      highlightedElement &&
      highlightedElement.dataset.orianbuilderId === componentId
    ) {
      highlightedElement = null;
    }
  }
   ...
   * Returns the parent orianbuilder-tagged element if the current one is an overlay,
  ...
  function skipOverlayElement(el) {
    ...
    // This looks like an overlay — walk up to the parent with a orianbuilder-id
    let parent = el.parentElement;
    while (parent && !parent.dataset.orianbuilderId)
  ...
  function onMouseMove(e) {
    ...
    let el = e.target;
    while (el && !el.dataset.orianbuilderId) el = el.parentElement;
  ...
  function onClick(e) {
    ...
    const clickedComponentId = state.element.dataset.orianbuilderId;
    ...
    if (selectedItem && (highlightedElement === state.element || !isProMode)) {
      ...
      window.parent.postMessage(
        {
          type: "orianbuilder-component-deselected",
    ...
    if (!state.element.dataset.orianbuilderRuntimeId) {
      state.element.dataset.orianbuilderRuntimeId = `orianbuilder-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    }

    const rect = state.element.getBoundingClientRect();
    window.parent.postMessage(
      {
        type: "orianbuilder-component-selected",
        component: {
          id: clickedComponentId,
          name: state.element.dataset.orianbuilderName,
          runtimeId: state.element.dataset.orianbuilderRuntimeId,
        },
        coordinates: {
          top: rect.top,
          left: rect.left,
  ...
  function onKeyDown(e) {
    ...
    if (key === "c" && hasShift && hasCtrlOrMeta) {
      ...
      window.parent.postMessage(
        {
          type: "orianbuilder-select-component-shortcut",
        },
        "*",
      );
    }
  ...
  window.addEventListener("message", (e) => {
    ...
    if (e.data.type === "orianbuilder-pro-mode") {
      isProMode = e.data.enabled;
    }
    if (e.data.type === "activate-orianbuilder-component-selector") activate();
    if (e.data.type === "deactivate-orianbuilder-component-selector")
      deactivate();
    if (e.data.type === "activate-orianbuilder-visual-editing") {
      activate();
    }
    if (e.data.type === "deactivate-orianbuilder-visual-editing") {
      deactivate();
      clearOverlays();
    }
    if (e.data.type === "clear-orianbuilder-component-overlays")
      clearOverlays();
    if (e.data.type === "update-orianbuilder-overlay-positions") {
    ...
    if (
      e.data.type === "remove-orianbuilder-component-overlay" ||
      e.data.type === "deselect-orianbuilder-component"
    ) {
      if (e.data.componentId) {
        removeOverlayById(e.data.componentId);
      }
    }
    if (e.data.type === "restore-orianbuilder-component-overlays") {
      const componentIds = e.data.componentIds;
      if (Array.isArray(componentIds)) {
        clearOverlays();
        for (const id of componentIds) {
          const el = document.querySelector(
            `[data-orianbuilder-id="${CSS.escape(id)}"]`,
          );
          if (el) {
  ...
  function initializeComponentSelector() {
    if (!document.body) {
      console.error(
        "OrianBuilder component selector initialization failed: document.body not found.",
      );
      return;
    }
    ...
    //
    // see: https://github.com/orianbuilder-sh/orianbuilder/issues/2231
    const INIT_TIMEOUT_MS = 60_000; // Wait up to 60 seconds for tagged elements
    let observer = null;
    let timeoutId = null;

    function checkForTaggedElements() {
      if (document.body.querySelector("[data-orianbuilder-id]")) {
        ...
        window.parent.postMessage(
          {
            type: "orianbuilder-component-selector-initialized",
          },
          "*",
        );
        console.debug("OrianBuilder component selector initialized");
        return true;
      }
      return false;
    }
    ...
    setTimeout(() => {
      ...
      console.debug(
        "OrianBuilder component selector waiting for tagged elements to appear...",
      );

      observer = new MutationObserver((mutations) => {
        // Filter mutations to only process relevant changes
        const hasRelevantMutation = mutations.some((mutation) => {
          // Attribute mutation on data-orianbuilder-id (already filtered by attributeFilter)
          if (mutation.type === "attributes") {
            return true;
          }
          // Check if any added nodes have data-orianbuilder-id
          if (mutation.type === "childList") {
            for (const node of mutation.addedNodes) {
              if (node.nodeType === Node.ELEMENT_NODE) {
                if (
                  node.hasAttribute("data-orianbuilder-id") ||
                  node.querySelector("[data-orianbuilder-id]")
                ) {
                  return true;
                }
              }
      ...
      observer.observe(document.body, {
        ...
        subtree: true,
        attributes: true,
        attributeFilter: ["data-orianbuilder-id"],
      });
      ...
      timeoutId = setTimeout(() => {
        ...
        if (!document.body.querySelector("[data-orianbuilder-id]")) {
          console.warn(
            "OrianBuilder component selector not initialized because no DOM elements were tagged",
          );
        }
      }, INIT_TIMEOUT_MS);
    }, 0);
