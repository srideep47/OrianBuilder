(() => {
  ...
  function findElementByOrianBuilderId(orianbuilderId, runtimeId) {
    ...
    // Fall back to finding by orianbuilder-id (will get first match)
    const escaped = CSS.escape(orianbuilderId);
    return document.querySelector(`[data-orianbuilder-id="${escaped}"]`);
  }
  ...
  function applyStyles(element, styles) {
    ...
    console.debug(
      `[OrianBuilder Visual Editor] Applying styles:`,
      styles,
      "to element:",
      element,
    );
  ...
  function handleGetStyles(data) {
    ...
    const element = findElementByOrianBuilderId(elementId, runtimeId);
    if (element) {
      ...
      window.parent.postMessage(
        {
          type: "orianbuilder-component-styles",
          data: styles,
        },
        "*",
      );
  ...
  function handleEnableTextEditing(data) {
    ...
    const element = findElementByOrianBuilderId(componentId, runtimeId);
    if (element) {
      ...
      const onInput = () => {
        ...
        window.parent.postMessage(
          {
            type: "orianbuilder-text-updated",
            componentId,
            text: currentText,
          },
          "*",
      ...
      const cleanup = () => {
        ...
        window.parent.postMessage(
          {
            type: "orianbuilder-text-finalized",
            componentId,
            text: finalText,
          },
          "*",
  ...
  function handleGetTextContent(data) {
    ...
    window.parent.postMessage(
      {
        type: "orianbuilder-text-content-response",
        componentId,
  ...
  function handleModifyImageSrc(data) {
    ...
    if (imgEl) {
      ...
      if (imgEl._orianbuilderAbort) imgEl._orianbuilderAbort.abort();
      const controller = new AbortController();
      imgEl._orianbuilderAbort = controller;
      ...
      imgEl.addEventListener(
        ...
        () => {
          ...
          window.parent.postMessage(
            {
              type: "orianbuilder-image-load-error",
              elementId,
              src,
            },
            "*",
  ...
  window.addEventListener("message", (e) => {
    ...
    switch (type) {
      case "get-orianbuilder-component-styles":
        handleGetStyles(data);
        break;
      case "modify-orianbuilder-component-styles":
        handleModifyStyles(data);
        break;
      case "enable-orianbuilder-text-editing":
        handleEnableTextEditing(data);
        break;
      case "disable-orianbuilder-text-editing":
        handleDisableTextEditing(data);
        break;
      case "get-orianbuilder-text-content":
        handleGetTextContent(data);
        break;
      case "modify-orianbuilder-image-src":
